...
export default function SharedChat() {
  ...
  const isThinkingEnabled =
    thinkingMode === "thinking" || (thinkingMode === "hybrid" && enableThinking)
  ...
  useEffect(() => {
    ...
    checkConversationParticipant(conversationId)
      .then(() => {
        setJoined(true)
      })
      .catch(() => {
    ...
    const connect = async () => {
      try {
        ...
        const historical: Message[] = (
          (conversationData.messages ?? []) as ConversationHistoryMessage[]
        ).map(m => {
      ...
      ws.onmessage = event => {
        ...
        if (data.type === "message") {
          ...
          const updated = messagesRef.current.map(m =>
            m.id === pendingIdRef.current ? { ...m, content: pendingContentRef.current } : m,
          )
