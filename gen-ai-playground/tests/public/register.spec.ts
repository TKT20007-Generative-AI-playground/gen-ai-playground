...
test.describe('Register', () => {
  ...
  test('shows live password validation errors and clears when fixed', async ({ page }) => {
    await gotoRegisterPage(page);
    ...
    await expect(page.getByText('Password must contain at least one uppercase letter')).toBeHidden();
    await expect(page.getByText('Password must contain at least one number')).toBeHidden();
    await expect(page.getByText('Password must contain at least one special character')).toBeHidden();
  });

  test('invalid password blocks submit and does not call backend', async ({ page }) => {
  await gotoRegisterPage(page);
  ...
  await submitRegister(page);
  await expect(
    page.getByRole('alert').getByText('Password must be at least 8 characters long')
  ).toBeVisible();
  expect(requests.length).toBe(0);
});

  test('successful registration returns home and opens login modal', async ({ page }) => {
    await gotoRegisterPage(page);
  ...
  test('backend error is surfaced (invalid invitation code)', async ({ page }) => {
    await gotoRegisterPage(page);
