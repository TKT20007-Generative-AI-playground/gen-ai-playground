...
test('login text boxes can be filled', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Login' }).click();
  const dialog = page.getByRole('dialog', { name: 'Login' });
  await expect(dialog.getByRole('textbox', { name: 'Username' })).toBeEmpty();
  await dialog.getByRole('textbox', { name: 'Username' }).fill('user123');
  await expect(dialog.getByRole('textbox', { name: 'Username' })).toHaveValue('user123');
  await expect(dialog.getByRole('textbox', { name: 'Password' })).toBeEmpty();
  await dialog.getByRole('textbox', { name: 'Password' }).fill('1234');
  await expect(dialog.getByRole('textbox', { name: 'Password' })).toHaveValue('1234');
});
...
test('shows error on invalid credentials (alert dialog)', async ({ page }) => {
  ...
  await loginDialog.getByTestId('login-username').fill('nonexistent-user');
