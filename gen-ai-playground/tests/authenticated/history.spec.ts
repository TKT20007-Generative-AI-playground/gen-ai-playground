    ...
    images: [
      {
        ...
        image_data: 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR4nGNgYAAAAAMAASsJTYQAAAAASUVORK5CYII=',
        image_type: 'png',
      },
    ],
  },
  ...
  test.beforeEach(async ({ page }) => {
    await page.route('**/images/history', async (route) => {
      await route.fulfill({
        ...
        body: JSON.stringify({ history: dummyHistory.flatMap(g => g.images) }),
      });
    });
    ...
    // Tests
    test('opens history drawer and displays grouped prompts', async ({ page }) => {
    ...
    test('opens image modal when clicking on an image', async ({ page }) => {
        ...
        await expect(modalImg).toHaveAttribute('src', `data:image/png;base64,${base64}`);
    });
